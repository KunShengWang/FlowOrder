...
5. 设计不可变 Proposal -> 人工审批 -> 幂等执行 -> 业务回查的受控恢复闭环，由 FlowOrder 统一持有 Proposal 和 Action 事实；通过 `actionRequestId`、`EXECUTING` 租约和 reconciliation 处理响应丢失与崩溃窗口，并区分命令状态与业务结果状态。
...
| OrderCare M3 故障恢复 | 3 条租约测试 + 5 条对账测试；恢复域共 42 条通过，真实 HTTP/MQ E2E 验证 Action 查询和重复 reconcile |
...
- `docs/reports/ordercare/m3-action-reconciliation.md`
...
死信先落库到 `fo_mq_dead_letter`。FlowOrder 创建不可变 Proposal，固化目标、版本、状态指纹、影响与警告摘要和有效期；人工审批绑定这份具体预演。执行使用独立 `actionRequestId` 和 Action 租约：活跃 EXECUTING 拒绝第二执行者，租约过期且死信仍 PENDING 时才能用原 ID CAS 接管；业务已收敛但动作日志未完成时补记 SUBMITTED，无法证明时转 MANUAL_REVIEW。`SUBMITTED` 只代表命令可靠提交，只有业务回查通过才是 `RESOLVED`。

### Q10：项目现在最大的不足是什么？

我不会把它说成生产级高可用系统。当前已经在真实 MySQL、RabbitMQ 和服务进程上证明受控恢复、Action 查询与重复对账，并用自动化测试覆盖 EXECUTING 租约过期接管；enterprise-agent 侧也完成响应丢失和进程恢复证据。仍未完成真实多节点网络分区、可信服务身份、容量 SLO 和完整监控告警。
...
- 设计不可变 Proposal -> 人工审批 -> 幂等执行 -> 业务回查的受控恢复闭环，分离 proposalId 与 actionRequestId；通过 EXECUTING 租约和原 Action 对账处理结果未知，并将命令提交状态与业务收敛结果分开建模。
```
