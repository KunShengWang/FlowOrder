...
public class MqOutboxAdminController {
    ...
    @GetMapping("/dead")
    public ApiResponse<List<MqOutboxAdminDto>> findDead(@RequestParam(defaultValue = "100") int limit) {
        return ApiResponse.success(outboxService.findDead(limit));
    }

    @PostMapping("/dead/{messageId}/retry")
    public ApiResponse<Void> retryDead(@PathVariable String messageId) {
