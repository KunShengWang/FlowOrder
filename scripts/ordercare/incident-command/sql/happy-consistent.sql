DROP PROCEDURE IF EXISTS inject_incident_happy;
DELIMITER $$
CREATE PROCEDURE inject_incident_happy()
BEGIN
    ...
    INSERT INTO fo_stock_item (
        ...
        9100000000000001, 'IC-HAPPY-STOCK', 1, 'Incident Command happy fixture',
        100, 97, 3, 0, 1, 0, NOW(), NOW(), 0
    );

    WHILE i <= 3 DO
        SET req = CONCAT('IC-HAPPY-REQ-', LPAD(i, 3, '0'));
        SET ord = CONCAT('IC-HAPPY-ORD-', LPAD(i, 3, '0'));
        SET deduct = CONCAT('IC-HAPPY-DED-', LPAD(i, 3, '0'));
        ...
        INSERT INTO fo_reservation_request (
            ...
            9100000000010000 + i, req, CONCAT('IC-HAPPY-TRACE-', i), 9100000000000100 + i,
            1, 9100000000000001, 1, ord, 20, 40, 1, 0, 0, NOW(), NOW()
        );
        ...
        INSERT INTO fo_reservation_order (
            ...
            9100000000020000 + i, ord, 9100000000000100 + i, 1, 9100000000000001, 1,
            40, req, deduct, DATE_SUB(NOW(), INTERVAL 5 MINUTE),
            'Incident Command happy fixture', 1, NOW(), NOW(), 0
        );
        ...
        INSERT INTO fo_stock_deduct_record (
            ...
            9100000000030000 + i, deduct, ord, 9100000000000100 + i, 1, 9100000000000001,
        ...
        INSERT INTO fo_mq_dead_letter (
            ...
            9100000000040000 + i, CONCAT('IC-HAPPY-MSG-', LPAD(i, 3, '0')),
            ...
            'Incident Command deterministic happy fixture', 0, 0, NOW(), NOW()
        );
        SET i = i + 1;
    END WHILE;
END$$
DELIMITER ;
CALL inject_incident_happy();
DROP PROCEDURE inject_incident_happy;

SELECT 3 AS terminalOrderCount, 3 AS unreleasedDeductCount,
       3 AS deadLetterRecordCount, 3 AS distinctDeadLetterRequestIdCount,
       'HAPPY_CONSISTENT' AS fixture;
