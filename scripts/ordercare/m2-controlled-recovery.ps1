...
function Invoke-MavenTests([string]$Tests) {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "M2 tests failed with exit code $LASTEXITCODE"
        }
    }
    finally {
        Pop-Location
...
switch ($Action) {
    'Verify' { Invoke-MavenTests 'RecoveryProposalServiceImplTest' }
    'E2E' { Invoke-E2E }
    'All' {
        Invoke-MavenTests 'RecoveryProposalServiceImplTest'
        Invoke-E2E
    }
